...
class PriorMonthService:
    ...
    def build_prior_context(self, prior_run_id: int) -> dict[str, Decimal]:
        ...
        # Tape values
        for ev in self.db.query(ExtractedValue).filter(ExtractedValue.run_id == prior_run_id).all():
            if ev.parsed_value is not None:
                context[f"{ev.variable_name}_prior"] = ev.parsed_value
        # Execution steps
        for step in self.db.query(ExecutionStep).filter(ExecutionStep.run_id == prior_run_id).all():
    ...
    def get_default_priors(self, deal_id: int) -> dict[str, Decimal]:
        ...
        nodes = (
            ...
            .filter(DagNode.dag_version_id == current_version.id, DagNode.default_prior_value.isnot(None))
            .all()
        )
