...
def test_build_context_combined(db):
    ...
    assert ctx["class_a_note_rate"] == Decimal("0.0425")
...
def test_build_context_with_split(db):
    ...
    ctx = TrancheService(db).build_tranche_context(deal.id, "2026-04")
    assert ctx["class_a_balance"] == Decimal("182255600")
    assert ctx["class_a_balance_144a"] == Decimal("100000000")
    assert ctx["class_a_balance_regs"] == Decimal("82255600")
...
def test_build_context_prior_month(db):
    ...
    assert ctx["class_a_balance"] == Decimal("180000000")
    assert ctx["class_a_balance_prior"] == Decimal("185000000")
